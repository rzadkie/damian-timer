...
export default function Torch({diff, min, sec}){
    ...
    class FireMaterial extends THREE.ShaderMaterial {
        constructor() {
          super({
            defines: { ITERATIONS: '5', OCTIVES: '2' },
            uniforms: {
      ...
      function Fire({ color, ...props }) {
        const ref = useRef()
        const texture = useLoader(THREE.TextureLoader, '/fire2.png')
        useFrame((state) => {
    ...
    return(
        <div className="Torch">
            ...
            {/* <p>{min}:{sec}</p> */}
            <Canvas style={{width:`100px`, height:`100px`, zIndex:'15'}} className="FireCanv" >
                <Fire scale={new THREE.Vector3(3, 2, 3)}  />
