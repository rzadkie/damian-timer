...
import { collection_mechanics, grp, col } from "../services/firebase";
...
const Incrementer = () => {
	...
	useEffect(() => {
		const update = onSnapshot(collection_mechanics, (snapshot) => {
			console.log(snapshot.docs);
			//setValue(snapshot.docs.map(user => ({...user.data()})))
		});
		return () => {
			update();
		};
	...
	const updateValue = async () => {
		try {
			await firebase.firestore().collection("characters").doc(stats).update({
				light: value,
			});
		} catch (error) {
	...
	return (
		<div className="Background">
			...
			<div className="Background2">
				...
				<div className="BTNsR">
					<button
						...
						disabled={value === 0}
						onClick={() => {
							setValue(value + diffLevel.value[0]);
							updateValue();
						}}
					>
						{" "}
					...
					<button
						...
						disabled={value === 0}
						onClick={() => {
							setValue(value + diffLevel.value[1]);
							updateValue();
						}}
					>
						{" "}
					...
					<button
						...
						disabled={value === 0}
						onClick={() => {
							setValue(value + diffLevel.value[2]);
							updateValue();
						}}
					>
						{" "}
					...
					<button
						...
						disabled={value === 0}
						onClick={() => {
							setValue(value + diffLevel.value[3]);
							updateValue();
						}}
					>
						{" "}
					...
					<button
						className="BigButton"
						disabled={value + 25 >= 100}
						onClick={() => {
							setValue(value + diffLevel.value[4]);
							updateValue();
						}}
					>
						{" "}
			...
			<FearStat diff={expValue} difficulty={diffLevel.fright} groups={groups} />
		</div>
	);
};
